...
class CdePanel(QtGui.QWidget):
    def __init__(self, opts, parent=None):
        ...
        firstrunfile=Globals.configdir+'/firstrun'
        if not os.path.isfile(firstrunfile):
            with open(firstrunfile, 'w') as f:
                f.write('firstrun')
            self.saveTimer=QtCore.QTimer()
            self.saveTimer.setSingleShot(True)
            self.saveTimer.timeout.connect(self.iconCacheMessageBox)
            self.saveTimer.start(1)
    ...
    def iconCacheMessageBox (self):
        print 'MESSAGE'
        msg = QtGui.QMessageBox()
        msg.setIcon(QtGui.QMessageBox.Information)
        msg.setText("Converting icons to CDE style on initial run. May slow down opening the drawers for a couple of minutes...")
    ...
    def findAppBrowser(self):
        ...
        #20221106 look at this some time for custom item for the applauncher
...
#__1
    ...
    GenDefaultDrawersAndLayout.generate(configdiruserhome)
    #no longer necessary... WELL maybe for initial panel height??
...
#NOT USED
#20221106 NOW DONE DIFFERENTLY BUT KEEP FOR NOW
#some initial guess for config on the first run
#maybe better do this for initial height=faction of screenheight? 
#now it is scale 1
...
def main():
    print 'starting cdepanel VER 1.4...'


    print 'checking for some required executables..'
    if not cmd_exists('convert'):
        print """Image magick convert not found. Please do 'sudo apt install imagemagick' or something similar """
    ...
    #damn why this preload error in appimage????
    #print 'LD_PRELOAD:'
...
#__1
    ...
    #see also writeConfigFileForFirstRun
    ...
    #print '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'
    #hier
    #print ">"+str(Globals.font)+"<"
    #print ">"+str(Globals.fontStyle)+"<"
    #print ">"+str(Globals.fontSize)+"<"
