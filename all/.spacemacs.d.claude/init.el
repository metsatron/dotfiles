...
(defun dotspacemacs/user-config ()
  ...
  Falls back to *scratch* so the layout is never left showing the Claude buffer
  twice when invoked from inside the Claude window."
  ...
  See the prose above for why."
    ...
    ;; Return to the editor: the sole window with no `window-side' parameter.
    (when-let ((editor (seq-find (lambda (w) (not (window-parameter w 'window-side)))
                                 (window-list))))
      (select-window editor))
    ;; Hand off to claude-code-ide's own display path — see the prose above for
    ;; why anything else silently drops the width.
    (let ((claude-code-ide-use-side-window t)
          (claude-code-ide-window-side 'right)
          (claude-code-ide-window-width metsatron/claude-workspace-width))
      (let* ((name (and (fboundp 'claude-code-ide--get-buffer-name)
