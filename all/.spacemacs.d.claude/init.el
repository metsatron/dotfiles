...
(defun dotspacemacs/user-config ()
  ...
  Not interactive on purpose — reach it through the @Claude layout. See below."
    ...
    ;; Tell the PTY its new size. vterm only learns its dimensions from the window,
    ;; and after the splits above the CLI is still rendering for the OLD width — it
    ;; wraps at the wrong column and paints the same text twice at two different
    ;; widths, which looks like garbled interleaved output.
    ;;
    ;; `claude-code-ide--sync-terminal-dimensions' takes (BUFFER WINDOW). It was
    ;; called here with NO arguments, inside `ignore-errors' — so it raised
    ;; wrong-number-of-arguments and did nothing, silently, every single time. The
    ;; resync was a no-op for as long as this function has existed. Never wrap a
    ;; call in `ignore-errors' to make it "safe": that suppresses exactly the error
    ;; that tells you the call is wrong. Guard the CONDITION, not the CALL.
