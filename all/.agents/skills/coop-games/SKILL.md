...
- **Canonical state outranks appearance.** In Pokemon or link-cable work, never assign a trainer, player, consort, partner, or species from a screenshot, model/provider name, process name, or UI context. Resolve identity from the canonical state surface; if it is absent, report it unresolved. A prospective assignment or live roster is not a minted save, and those states must not be collapsed.
