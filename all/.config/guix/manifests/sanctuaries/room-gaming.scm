...
(packages->manifest
 (cons* pcmanfm-redstone
   ...
   ;; These ARE KF6 apps, so they carry the exact gap that got Ark removed one
   ;; line above: KF6 reads its palette from KColorScheme/kdeglobals, not from
   ;; qt6ct, so they do not inherit the scheme's Qt palette the way Audacious
   ;; and SMPlayer do. Added deliberately anyway — they are the games asked for
   ;; and there is no GTK equivalent of KMahjongg/KShisen worth substituting.
   ;; Mitigated with a generated kdeglobals (see redstone-9x-scheme-apply);
   ;; without it they render in stock Breeze colours against a Win9x desktop.
