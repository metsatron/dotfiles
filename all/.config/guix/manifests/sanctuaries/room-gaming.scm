...
;; wrong for Redstone's desktop build stamp: IceWM cannot manage or layer an
;; override-redirect window, so it remains above ordinary applications.  This
;; narrow room-local variant keeps osd_cat's transparent shaped rendering while
;; exposing a stable WM_CLASS, letting IceWM place it in the Below layer.  The
;; empty input shape makes the text click-through.
              ...
              (lambda _
                (substitute* "src/libxosd/xosd.c"
                  ...
                  (("setwinattr.override_redirect = 1;")
                   "setwinattr.override_redirect = 0;")
                  ...
                  (("  stay_on_top\\(osd->display, osd->window\\);")
                   "  /* IceWM owns stacking for the Redstone build stamp. */")
                  (("      XMoveWindow\\(osd->display, osd->window, x, y\\);")
                   (string-append
                    ...
                    "        size_hints.x = x;\n"
                    "        size_hints.y = y;\n"
                    "        size_hints.width = osd->screen_width;\n"
                    "        size_hints.height = osd->height;\n"
                    "        XSetWMNormalHints(osd->display, osd->window, &size_hints);\n"
                    "        XMoveWindow(osd->display, osd->window, x, y);\n"
                    "      }"))
                ...
                (invoke "grep" "-q" "setwinattr.override_redirect = 0" "src/libxosd/xosd.c")
                (invoke "grep" "-q" "RedstoneBuildstamp" "src/libxosd/xosd.c")
                (invoke "grep" "-q" "XSetWMNormalHints" "src/libxosd/xosd.c")
