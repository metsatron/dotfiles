...
def main():
    ...
    try:
        while True:
            ...
            if b"/compact" in data and "--ignore-compact" not in sys.argv:
...
class CodexWarmTests(unittest.TestCase):
    ...
    def test_compaction_stop_recovers_and_rearms_without_post_compact(self):
        session = self.make_session(delay=60)
        session.stop_and_bind()
        session.control("trigger")
        session.wait_output("COMPACT_RECEIVED")
        wait_until(lambda: session.read_state() and session.read_state()["compacting"])
        result = session.event("stop", stop_hook_active=False)
        self.assertEqual(result.get("ignored"), "compaction-stop")
        wait_until(
            lambda: session.read_state()
        ...
        self.assertEqual(state["last_state_transition"], "compaction-recovered")

    def test_compaction_timeout_recovers_and_rearms_without_post_compact(self):
        session = self.make_session(
            delay=60,
            compaction_timeout=0.2,
            extra_args=("--ignore-compact",),
        )
        session.stop_and_bind()
        session.control("trigger")
        session.wait_output("INPUT:")
        wait_until(
            lambda: session.read_state()
            and not session.read_state()["compacting"]
            and session.read_state()["timer_deadline"] is not None,
            timeout=3,
            message="compaction timeout did not recover and re-arm",
        )
        state = session.read_state()
        self.assertTrue(state["dirty"])
        self.assertEqual(state["last_injection_result"], "failed")
        self.assertEqual(state["last_state_transition"], "compaction-recovered")
    ...
    def test_codex_screen_classifier_mirrors_visible_detector_evidence(self):
        classifier = self.module.CodexScreenClassifier()
        self.assertEqual(classifier.feed(b"\x1b]0;Codex\x07"), "idle")
        self.assertEqual(classifier.feed("\x1b]0;⠹ Codex\x07".encode()), "working")
        self.assertEqual(
            classifier.feed(b"\x1b]0;Action Required\x07"),
            "blocked",
        )
        classifier.reset()
        self.assertEqual(
            classifier.feed(b"\x1b]0;Codex\x07allow command?"),
            "blocked",
        )
