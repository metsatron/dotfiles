...
# Claude Code for writes and refactors (one-shot guard mandatory — see claude-code skill hard rule)
claude --strict-mcp-config --mcp-config '{"mcpServers":{}}' -p "PROMPT" --model claude-sonnet-5 --output-format json

# Codex for quick tasks
codex exec - --full-auto --skip-git-repo-check --json -c model="gpt-5.4-mini" <<< "PROMPT"

# Claude Code for quick tasks (same guard)
claude --strict-mcp-config --mcp-config '{"mcpServers":{}}' -p "PROMPT" --model claude-3.5-haiku --output-format json
```
...
```bash
claude --strict-mcp-config --mcp-config '{"mcpServers":{}}' -p "PROMPT_HERE" --output-format json
```

**HARD RULE (sealed 2026-08-19): every non-interactive `claude -p` MUST carry `--strict-mcp-config --mcp-config '{"mcpServers":{}}'`.** Without it the nested claude loads the full plugin set, and a freshly spawned telegram plugin SIGTERMs the live session's poller via the shared `bot.pid` before claiming the token — one-shot calls were assassinating live sessions' Telegram channels (the 2026-08-19 "plugin keeps disconnecting" hunt). Plugins and channels belong only to real interactive sessions; a one-shot gets a model and nothing else.
...
| `--strict-mcp-config --mcp-config '{"mcpServers":{}}'` | MANDATORY for one-shots — no plugins/MCP (see hard rule above) |
...
```bash
# The mandatory one-shot guard, spelled once for copy-paste
GUARD=(--strict-mcp-config --mcp-config '{"mcpServers":{}}')

# Basic non-interactive
claude "${GUARD[@]}" -p "Explain what this function does" --output-format json < src/auth.py

# With model selection
claude "${GUARD[@]}" -p "Refactor for clarity" --model claude-sonnet-5 < src/auth.py

# Read-only analysis (restrict tools)
claude "${GUARD[@]}" -p "Review for security issues" --allowedTools "Read,Grep,Glob" --output-format json

# With turn limit
claude "${GUARD[@]}" -p "Write unit tests" --max-turns 5

# Pipe content
cat src/auth.py | claude "${GUARD[@]}" -p "Add type annotations"
```
