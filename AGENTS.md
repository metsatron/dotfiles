...
8. **All agent work lives in `.org` files** -- skills, hooks, plugins, custom instructions, and settings are authored in org source at repo root and emitted by tangle. Never write directly to harness directories. This includes:
   - New skills: add to `agents-skills.org` with the correct tangle target(s)
...
9. **Skills over commands** -- skills are universal across harnesses; harness-specific commands are not portable. Add new agent capabilities as skills in `agents-skills.org`, not as commands.
...
Skills are authored in =agents-skills.org= with one of three emission modes:
...
All other skills in =agents-skills.org= are stow-global.
...
- `agents-skills.org` -- all agent skills (universal and harness-exclusive)
...
3. **Skills are authored in `agents-skills.org`** -- tangle to `all/.agents/skills/X/SKILL.md` for stow-global skills, `.agents/skills/X/SKILL.md` for DotCortex-scoped skills, or `.opencode/skills/X/SKILL.md` for OpenCode-exclusive skills. Do not write skill files directly.
