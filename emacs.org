...
currently in — the workbench, say — completely intact. The first version of this
was an interactive command bound to =SPC o W= / =C-c W= that ran
=delete-other-windows= on the current frame; one mistyped shift key away from the
workbench's =SPC o w= / =C-c w=, it demolished the live workbench on contact.
Two lessons, both paid for: a "new workspace" belongs in the workspace system
(=spacemacs|define-custom-layout=), not in a function that rearranges the frame
you are standing in; and a destructive command must never sit one modifier away
from the thing it destroys.
