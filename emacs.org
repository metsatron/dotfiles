...
;;
;; An earlier version got this wrong twice over, and both are worth remembering:
;; it was an interactive command that called `delete-other-windows' on the
;; CURRENT frame, and it was bound to `SPC o W' / `C-c W' — exactly one shift
;; key away from the workbench's `SPC o w' / `C-c w'. A single mistyped key
;; therefore demolished the live workbench. A destructive command must not sit
;; one modifier away from the thing it destroys; do not re-add a global binding
;; for the builder.
