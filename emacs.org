...
(defvar metsatron/terminal-font-height 120
  "Face height for terminal buffers (1/10 pt), so 120 = 12pt.
Deliberately larger than the 11pt frame default: terminal buffers are read as
running output, not scanned as code. 120 is not arbitrary — it is the size
arrived at by eye with `text-scale-mode' (one step = 1.2x over the previous
100), made declarative here so it survives a restart and reaches every machine.
Raise or lower HERE rather than reaching for `text-scale-mode', which is
buffer-local, per-session, and dies with the Emacs process.")
