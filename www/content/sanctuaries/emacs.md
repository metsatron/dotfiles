...
The working sanctuary: a Guix-backed container built around a topology of independent Emacs processes — Qtile ruling the windows today, probably EXWM in the end. Inside a nested display the window manager is not the host's problem, so Emacs may as well own the whole X session. The topology: one for core work, one as a disposable launcher frame, one for dashboards, one for agent sessions and long-running tools, one for journals. Each has its own socket, state, and entrypoint. One shared constitution, multiple independent bodies: a wedged agent Emacs cannot freeze the launcher.
