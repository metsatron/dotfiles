...
probe() {
  local name="$1" expected="$2" os_release="$3" devuan_file="$4"
  local actual=0
  NALA_SCOPE_PROBE=1 NALA_OS_RELEASE="$os_release" NALA_DEVUAN_VERSION_FILE="$devuan_file" \
    NALA_ARCH=amd64 NALA_RELEASES_SSV="$ROOT/debian/.nala/manifest/releases.ssv" \
...
printf '%s\n' '#!/usr/bin/env bash' 'if [ -f "$NALA_TEST_STATE" ]; then printf "install ok installed\t%s\n" "$(cat "$NALA_TEST_STATE")"; else exit 1; fi' > "$stubbin/dpkg-query"
...
printf '%s\n' '#!/usr/bin/env bash' 'printf apt-get-f-called >> "$NALA_TEST_LOG"' '[ "${NALA_TEST_REPAIR:-0}" = 1 ] && printf 2 > "$NALA_TEST_STATE"' 'exit 0' > "$stubbin/apt-get"
chmod +x "$stubbin"/*
