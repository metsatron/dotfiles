...
* OpenRC boot owner tests

Run =python3 linux/.local/share/dotcortex/tests/test-claude-warm-openrc.py -v=. Every privileged tool is a stub on a private =PATH= and the target directories are temporary, so nothing is installed and no service is touched. The last test compares the rendered script for Honey's parameters with Honey's live =/etc/init.d/claude-warm= (comments stripped) and runs only on Honey.

#+BEGIN_SRC python :tangle linux/.local/share/dotcortex/tests/test-claude-warm-openrc.py :shebang "#!/usr/bin/env python3" :tangle-mode (identity #o755) :comments no
from __future__ import annotations
