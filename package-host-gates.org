...
- *Nothing outside the host's own list.* nala: the gated manifest replaces the shared fleet list (unless =NALA_SSV= is set explicitly). npm/cargo/bun/flatpak: a gated host uses only its *host lane*, =<manifest dir>/hosts/<hostname>.ssv=, or, when a second account on the host provisions its own per-user prefix, that account's lane =<manifest dir>/hosts/<hostname>@<account>.ssv= (=provision-gate lane <dir>= picks it; the account lane wins when it exists). With no host lane there is nothing to install, and the lane exits 0 without running anything. For flatpak a host lane would replace =apps.ssv= only; there is none for Honey.
...
ACCOUNT="${PROVISION_GATE_ACCOUNT:-$(id -un 2>/dev/null || echo "${USER:-}")}"
...
Usage: provision-gate status              show this host's gate
       ...
       provision-gate lane <manifest-dir> print the host lane for this host and account:
                                          <dir>/hosts/<host>@<account>.ssv if it exists,
                                          else <dir>/hosts/<host>.ssv
...
Env:   PROVISION_GATES (gate file; default: repo copy + ~/.provision/host-gates.ssv)
       ...
       PROVISION_GATE_ACCOUNT (default: id -un)
...
case "$cmd" in
  ...
  host) echo "$HOST" ;;
  lane)
    dir="${2:?Usage: provision-gate lane <manifest-dir>}"
    if [ -n "$ACCOUNT" ] && [ -f "$dir/hosts/$HOST@$ACCOUNT.ssv" ]; then
      echo "$dir/hosts/$HOST@$ACCOUNT.ssv"
    else
      echo "$dir/hosts/$HOST.ssv"
    fi
    ;;
  manifest)
...
- *npm:* =package-npm.org= "Host lane: Honey (beelink)", tangled to =all/.npm/manifest/hosts/beelink.ssv=, holding the 4 globals in Honey's =~/.local= prefix. The =metsatron= account has its own lane, =all/.npm/manifest/hosts/beelink@metsatron.ssv= (Claude Code and bun for his Telegram bots, prefix =/home/metsatron/.npm-global=), so an npm run as =metsatron= never plans Gillean's globals and one as =gille= never plans his.
- *Node for =metsatron= (proposed, not active):* the fleet's Node >= 22 is Guix, not apt. Surveyed read-only on 2026-09-26: kikin and the T480 run node 24.18.0 from the Guix =core= profile, with Claude Code in =~/.npm-global=; the T480s has the same Guix profile, but its PATH resolves an unmanaged =~/.local/bin/node= (v24, a June copy) and a NodeSource apt repo that only offers 20.20 (drift, not a lane). Devuan offers 20.19 only. Honey has no Guix (no =/gnu=, no daemon). The DotCortex-native route is therefore: (1) the one-time Guix bootstrap from [[file:package-guix.org][package-guix.org]] on Honey, which is systemd-only and =:tangle no= today, so it first needs an OpenRC =guix-daemon= service *encoded in DotCortex* (the T480's is a hand-written native OpenRC script, kikin's and the T480s's hand-written LSB scripts; none is tracked); (2) a node-only profile for =metsatron= built from a tracked manifest (like =agents/claude.scm=) through a gated Honey Guix lane (dry-run unless =PROVISION_APPLY=1=). Cost on Gillean's production box: a root daemon, the =guixbuild= group and build users, and =/gnu/store= (a node-only closure, far smaller than kikin's 28 GB store). Needs Mètsàtron's and Gillean's word before anything is written.
