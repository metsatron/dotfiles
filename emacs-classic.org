#+TITLE: Classic Sanctuary Emacs — vanilla, polished, and shared
#+DESCRIPTION: Shared vanilla Emacs profile for the non-Qtile classic sanctuaries
#+AUTHOR: Mètsàtron
...
This is the vanilla Emacs profile for the classic sanctuary family. It is
projected into every Distrobox sanctuary except =sanctuary-qtile=, whose
multi-Emacs setup remains independent. The host Doom and Spacemacs profiles are
not changed by this file.

The visual and interaction contract deliberately carries over the useful parts
of [[file:emacs-doom.org][emacs-doom.org]] and [[file:emacs-spacemacs.org][emacs-spacemacs.org]]: Evil, Colemak-NEIO, Org's
variable-pitch/fixed-pitch grammar, polished tables, inline images, and
predictable caches. Treemacs, tabs, Doom modules, and Spacemacs plumbing stay
out of this small vanilla profile.

The selected Org table behavior follows the existing Doom configuration's
"tables only" rule: =org-modern= paints tables, while headings, stars, blocks,
and emphasis remain ordinary Org surfaces with theme-inherited faces. This
avoids two independent prettifiers fighting over the same glyphs.

* Early init

#+BEGIN_SRC emacs-lisp :tangle all/.config/emacs-classic/early-init.el
...
#+BEGIN_SRC emacs-lisp :tangle all/.config/emacs-classic/init.el
...
#+BEGIN_SRC bash :tangle all/.local/bin/classic-emacs-theme-apply :shebang "#!/usr/bin/env bash" :tangle-mode (identity #o755)
