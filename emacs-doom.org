...
Codex-native current context usage, while =tokenUsage.total.totalTokens= is
cumulative.  The installed =emacs-codex-ide= owns identity and normalizes
status to =running=/=idle=.  Its =turn/completed= path emits a normalized
completion when present, but a privacy-safe 0.146.0 run reached =idle= without
that raw notification; the governor therefore also accepts completed normal
agent items followed by idle.  The optional raw =threadId= remains only an
ownership cross-check.  Codex's 30-minute public GPT-5.6 guidance is represented
as a configurable presumed warm-cache budget with heuristic/backend-unverified
confidence, never as an authenticated-backend TTL.  The generated 0.146.0
schema's optional =rawResponse/completed= notification carries a direct
=params.usage= object with Codex-normalized =cachedInputTokens=,
=cacheWriteInputTokens=, =inputTokens=, and =totalTokens= fields (or a null
usage).  Those exact upstream counters are telemetry only; absent counters are
unknown and do not establish a cache miss or a retention interval.
