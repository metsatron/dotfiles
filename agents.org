...
For sanctuary-sx this is tangled to =linux/.config/sanctuary-sx/autostart/sanctuary-sx-colemak.desktop= and projected into the guest home by =sanctuary-sx-launch=.

### XFCE 4.20 Wallpaper Folder-Picker Permanently Broken (Xephyr)

The folder picker in the XFCE 4.20 Background settings dialog fails silently when Xephyr reports the monitor name as =default= — the GUI writes settings under =monitor0= but xfdesktop reads from =monitordefault=. The wallpaper never changes no matter what you select.

Fix: use =sanctuary-sx-wallpaper <path>= script, which bypasses the GUI entirely by writing directly to xfconf with the correct key path (=/backdrop/screen0/monitordefault/workspace0/last-image=) via the guest's D-Bus session and then restarts xfdesktop. Never use the XFCE Background GUI for wallpaper changes inside sanctuary-sx.
