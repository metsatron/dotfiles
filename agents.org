...
- T480 reality: no system node/npm — the human's lives in a per-user Guix profile,
  ...
  a *per-agent Guix profile* (=guix install node= as the agent against the shared
  daemon) — pending, alongside the repo-access decision. No package was shortcut
  into place from the human's toolchain.
